...
/**
 * Encapsulates host downtime removal
 *
 * Allows a downtime to be remove from a specifc host as a shortcut or a
 * number of hosts as defined by a filter.
 */
class HostDowntimeRemove extends Command
{
    /**
    ...
    public function get_parser($name)
    {
        ...
        $group->add_argument('--host', 'Host to remove downtime for');
    ...
    public function execute($argv)
    {
        ...
        if($response['results']) {
            ...
            $attachment->set_title('Host Downtimes Removed');
            $attachment->set_color('#0095bf');
            $attachment->set_markdown_in('text');
            foreach($response['results'] as $result) {
                $host = explode('\'', $result['status'])[1];
